    ...
    <EditorWrapper>
      {editor.map((line, i) => {
        ...
        <EditorLine key={i}>
          ...
          {line.type === 'input' && <input value={cssInput} onChange={(e:InputEvent) => setCssInput(e.target.value)} aria-label={line.aria || line.content}/>}
          {line.type === 'input2' && <input value={cssInput2} onChange={(e:InputEvent) => setCssInput2 && setCssInput2(e.target.value)} aria-label={line.aria || line.content}/>}
        </EditorLine>
        )
      })}
    </EditorWrapper>
